...
router.get('/getThumbnail', async (req, res) => {
    try {
        ...
        const base = BUNNY_TOKEN_KEY + path + expires
        ...
        const imageUrl = `https://${BUNNY_CDN_HOST_NAME}${path}?token=${token}&expires=${expires}`
...
router.get('/player/:id', (req, res) => {
    ...
    const data = BUNNY_TOKEN_KEY + videoId + expiration

    const token = crypto.createHash('sha256').update(data).digest('hex')

    const secureUrl = `https://iframe.mediadelivery.net/embed/${BUNNY_LIBRARY_ID}/${videoId}?token=${token}&expires=${expiration}`
...
/*router.get('/api/auth/google/callback', 
    ...
    (req, res) => {
        ...
        res.redirect("/"); // Redirect to a dashboard or home after successful login
    }
);*/
router.get('/api/auth/google/callback', 
    passport.authenticate('google', { failureRedirect: '/ }), 
